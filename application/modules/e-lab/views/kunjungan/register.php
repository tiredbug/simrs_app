			...
			<div class="col-sm-7">
				<div class="form-group">
					...
					<div class="col-sm-9">
						...
						<input tabindex="7" class="" type="radio"  name="asal" value="rajal">
				...
				<div class="form-group">
					<label class="control-label col-sm-3">Dokter Lab :</label>
					<div class="col-sm-9">
...
<!-- modal produk lab -->
<div class="modal fade"  role='dialog' id="modal_lab">
...
<script type="text/javascript">
	...
	$(".norek").keypress(function(e)
	{
		...
		if(e.which==13)
		{
			...
			$.ajax({
				type:"POST",
				url:base_url+'e-lab/kunjungan_api/get_informasi_kunjungan',
				data:data,
				dataType:'json',
				error:function()
				{
				...
				success:function(json)
				{
					if(json.success)
					{
						...
						if(asal=='rajal')
						{
							...
							.end()
							.append('<option value="">-- Pilih --</option>')
							$.each(json.ruang,function( i , dat)
							{
								$("<option>").
								attr('value',dat.id).
								text('Poliklinik '+dat.poli).
								appendTo($(".unit"));
							})
						}
						loading('hide');
	...
	$(".unit").change(function()
	{
		...
		$.ajax({
			type:"POST",
			url:base_url+'e-lab/kunjungan_api/get_dokter_poli',
			data:'id='+id[0],
			dataType:'json',
			error:function()
			{
			...
			success:function(json)
			{
				...
				{
					loading('hide');
					toastr.error("Terjadi kesalahan sambunga.")
				}
			}
		})
	})
	...
	$(".btn-register").click(function(){
		...
		$.ajax({
			type:'POST',
			url:base_url+'e-lab/kunjungan_api/register_kunjungan',
			data:data,
			dataType:'json',
			error:function()
			{
	...
	$("#modal_lab").on('show.bs.modal',function(){
		$(".modal-body").html('')
		loading_modal('show')
		var norek=$(".norek").val();
		$.ajax({
			type:"POST",
			url:base_url+'e-lab/kunjungan_api/get_produk_lab_html',
			data:'norek='+norek,
			error:function(){
				toastr.error('Gagal terhubung ke server.')
				loading_modal('hide');
