...
<head>
    ...
    <title>Login - Pelayanan Rawat Inap</title>
</head>
<body>
    <div class="container">
        <div class="panel">
            ...
            <div class="panel-body">
                <form action="" method="post" class="form-login">
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <button type="submit" name="login" class="col-sm-2 pull-right btn btn-block btn-login "><b>Sign In <i class="entypo-login"></i></b></button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        $('.form-login').submit(function(e){
            ...
            $.ajax({
                type:"POST",
                url:'<?php echo base_url()?>e-ranap/login/login_auth',
                data:form_data,
                dataType:"JSON",
                error:function(xhr, desc, err)
                {
                    no_loading()
                    swal("Gagal",'gagal autentikasi')
                },
                success:function(json)
                {
                    if(json.success)
                    {
                        window.location.href='<?php echo base_url().'e-ranap'?>';
                    }
                    else
                    {
