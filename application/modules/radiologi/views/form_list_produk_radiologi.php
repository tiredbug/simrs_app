...
foreach ($tab->result() as $t) {
	...
	echo"'>
		<a href='#".$t->id."' data-toggle='tab'>
			...
			<span class='hidden-xs'>".$t->nama_group."</span>
		</a>
	</li>";
	$no++;
}?>
...
<div class='tab-content scroll'>
	...
	foreach ($tab->result() as $t_c) {
		$paket=$this->m_kunjungan->get_tab_conten($t_c->id);
		echo"
		<div class='tab-pane  ";
		echo $no==1?'active':'';
		echo"' id='".$t_c->id."'>";
						...
						<div class='text'>
							<div class='row'>
								<div class='col-sm-9 bold'>
									<input type='checkbox' onchange='pilih(this)' value='".$list->kode_tindakan."' ";
									echo $this->m_kunjungan->cek_stt_check($list->kode_tindakan,$norek)->num_rows() > 0?'checked':'';
									echo">
										".$list->tindakan."
								</div>
								<div class='col-sm-3 '>
...
<script type="text/javascript">
	function pilih(el) {
		...
		$.ajax({
			type:"POST",
			url:base_url+'radiologi/kunjungan_api/chart_layanan',
